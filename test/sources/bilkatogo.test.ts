/**
 * Unit tests for the BilkaToGo (Salling) source.
 *
...
import { bilkatogo } from '../../src/sources/bilkatogo.js';
import {
  algoliaSearchUrl,
  type AlgoliaHit,
...
const APP_ID = 'TESTBILKATOGO';
const PATH = 'bilkatogo';
const KEY = 'test-key-bilkatogo';

describe('bilkatogo source', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.BILKATOGO_PATH;
    delete process.env.BILKATOGO_APP_ID;
    delete process.env.BILKATOGO_KEY;
  });
  ...
  it('yields nothing when env is unset', async () => {
    ...
    const rows = await collect(bilkatogo());
    expect(rows).toEqual([]);
  });

  it('yields nothing when only some env vars are set', async () => {
    process.env.BILKATOGO_PATH = PATH;
    process.env.BILKATOGO_APP_ID = APP_ID;
    const stub: FetchLike = async () => {
  ...
  it('hits the documented Algolia URL with source=bilkatogo', async () => {
    process.env.BILKATOGO_PATH = PATH;
    process.env.BILKATOGO_APP_ID = APP_ID;
    process.env.BILKATOGO_KEY = KEY;
  ...
  it('emits Observations with source="bilkatogo" and currency="DKK"', async () => {
    ...
    for (const r of rows) {
      expect(r.source).toBe('bilkatogo');
  ...
  it('throws on a non-2xx response', async () => {
    ...
    await expect(collect(bilkatogo())).rejects.toThrow(/HTTP 503/);
  });
});